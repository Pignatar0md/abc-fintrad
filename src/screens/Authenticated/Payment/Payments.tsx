...
const Payments: FC<PaymentsScreenNavigationProp> = ({ navigation }): ReactElement => {
      ...
      header: () => (
        <>
          <View style={[styles.headerWrapper, { marginTop: insets.top }]}>
            <BackButton color={colors.grey.A100} text={''} onPress={() => navigation.goBack()} />
            <View style={styles.headerContentWrapper}>
              <Text style={styles.headerText}>{t('paymentsCapital')}</Text>
              <IconButton
  ...
  const getPaymentsList = async () => {
    ...
    if (response?.data?.Succeeded) {
      ...
      await sendInfoLog({ event: 'PaymentsList', detail: response?.data?.Succeeded });
    } else {
      if (response?.data === 'Request failed with status code 401') {
        ...
        await sendErrorLog({
          event: 'getPaymentsList getPayments - PaymentsList',
        ...
        <>
          ...
          {rowPressed === item.ID.toString() && (
            <View style={styles.extraInfoWrapper}>
              ...
              <View style={styles.rowExtraInfoWrapper}>
                <CellExtraInfoWrapper title={t('valueDateMayusc')} info={formattedDate} />
              </View>
              <View style={styles.rowExtraInfoWrapper}>
