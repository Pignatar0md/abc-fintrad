...
const AllBalances: FC<{ navigation: navigateType }> = ({ navigation }): ReactElement => {
  ...
  const renderItem = ({ item, index }: { item: CurrencyBalance; index: number }) => {
    const accountBalance = formatBalance(item.AccountBalance).toString();
    const spotBalance = formatBalance(item.SpotBalance).toString();
    const marginAC = item.MarginAC === 0 ? item.MarginAC.toFixed(2) : formatBalance(item.MarginAC);
    return (
      <Swipeable
  ...
  const renderHiddenItem = ({ item }: { item: { CCY: string; AccountBalance: number } }) => {
    ...
    return (
      <View style={styles.buttonsWrapper}>
        ...
        <ActionButton
          onPress={() =>
            navigation.navigate('Trade', { CCY: item.CCY, balance: item.AccountBalance })
          }
          buttonText={t('Trade')}
