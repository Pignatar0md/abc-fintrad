...
import { Appearance, FlatList, ScrollView, StyleSheet, View } from 'react-native';
...
const Negatives: FC<{ navigation: navigateType }> = ({ navigation }): ReactElement => {
  ...
  const renderHiddenItem = (item: { CCY: string; AccountBalance: number }) => {
    ...
    return (
      <View style={styles.buttonsWrapper}>
        ...
        />
        <ActionButton
          onPress={() => navigation.navigate('StatementsTopTabbar', { CCY: item.CCY })}
  ...
  if (balanceInfo?.negatives?.length === 0) {
    return (
