...
import { StatementsTopTabbarForMainNavigationProp } from 'types/types';
import { DEFAULT_HEADER_CONFIG } from 'utils/static';

const { grey, cyan } = colors;

const StatementsTopTabbarForMain: FC<StatementsTopTabbarForMainNavigationProp> = ({
  navigation,
  route,
}): ReactElement => {
  ...
  useLayoutEffect(() => {
    ...
    navigation.setOptions({
      ...
      header: () => (
        <>
          <BackHeader title={t('statementsCapital')} />
          <WrappedQuickLinks />
        </>
      ),
    });
...
export default StatementsTopTabbarForMain;
